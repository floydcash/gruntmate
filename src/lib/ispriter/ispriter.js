...
var collectStyleRules = function(styleSheet, result){
    ...
    for(var i = 0, rule, style, imageUrl, imagePath; rule = styleSheet.cssRules[i]; i++) {
        ...
        if(style['background-image'] && 
			...
			//如果不在需要合并的文件名称中，跳过
			if(spriteConfig.input.spriteMerge){
				
				var reg = new RegExp(spriteConfig.input.spriteMerge,"gi");
				
				if(!path.basename(imageUrl).match(reg))continue;
			}
...
var replaceAndPositionBackground = function(imageUrl, styleObj){
	...
	imageUrl = path.relative(spriteConfig.output.cssRoot,imageUrl).replace(/\\/g,"/");
...
exports.merge = function(configFile){
    onMergeStart();
    imageInfoCache = {};
    spriteConfig = typeof configFile=="object"?configFile:readConfig(configFile);
	...
	if(!spriteConfig.input.imageRoot){
        spriteConfig.input.imageRoot = spriteConfig.input.cssRoot;
    }
    if(!spriteConfig.output.imageRoot){
        spriteConfig.output.imageRoot = './image/';
    }
