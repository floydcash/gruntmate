...
task.removeTasks = function(){
	
	registry.tasks.length = 0;
	registry.untasks.length = 0;
...
function loadTask(filepath) {
  ...
  try {
    ...
    if (typeof fn === 'function') {
		...
		process.chdir(global.project_path);
...
task.loadNpmTasks = function(name) {
  loadTasksMessage('"' + name + '" local Npm module');
	...
	var exec_path = global.exec_path;
	
	//check is all plus model
  if(global.exec_path != global.project_path){
	
	var isExit = fs.existsSync(path.join(global.project_path,"node_modules",name));
	
	if(isExit)exec_path = global.project_path;
  }
  
  var root = path.resolve(path.join(exec_path,"node_modules"));
...
task.init = function(tasks, options) {
  ...
  var gruntfile = allInit ? null : grunt.option('gruntfile') ||
	...
	//console.log(grunt.file.findup('Gruntfile.{js,coffee}', {nocase: true}));
